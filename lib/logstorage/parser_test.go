...
func TestParseQuery_OptimizeOffsetLimitPipes(t *testing.T) {
	...
	f(`* | sort by (x) | fields a, b | limit 30`, `* | sort by (x) limit 30 | fields a, b`)
	f(`* | sort by (x) | delete a, b | offset 10 | limit 30`, `* | sort by (x) offset 10 limit 30 | delete a, b`)
	f(`* | sort by (x) | copy a as b | rename b as c | limit 30`, `* | sort by (x) limit 30 | copy a as b | rename b as c`)
	f(`* | sort by (x) | pack_json as json | pack_logfmt as logfmt | limit 30`, `* | sort by (x) limit 30 | pack_json as json | pack_logfmt as logfmt`)
	f(`* | sort by (x) | fields a | delete b | offset 10 | limit 30`, `* | sort by (x) offset 10 limit 30 | fields a | delete b`)
	f(`* | sort by (x) | fields a | limit 30 | offset 10`, `* | sort by (x) offset 10 limit 20 | fields a`)
	f(`* | sort by (x) | fields a | limit 0`, `* | limit 0 | fields a`)
	f(`* | sort by (x) | collapse_nums | offset 10 | limit 30`, `* | sort by (x) offset 10 limit 30 | collapse_nums`)
	f(`* | sort by (x) | unpack_json | limit 30`, `* | sort by (x) limit 30 | unpack_json`)
	f(`* | sort by (x) | fields foo | drop_empty_fields | limit 1`, `* | sort by (x) | fields foo | drop_empty_fields | limit 1`)
	f(`* | sort by (x) | extract "<foo>bar" keep_original_fields | limit 30`, `* | sort by (x) | extract "<foo>bar" keep_original_fields | limit 30`)
	f(`* | sort by (x) | extract_regexp "(?P<foo>bar)" skip_empty_results | limit 30`, `* | sort by (x) | extract_regexp "(?P<foo>bar)" skip_empty_results | limit 30`)
	f(`* | sort by (x) | filter a:foo | limit 30`, `* | sort by (x) | filter a:foo | limit 30`)
	f(`* | sort by (x) | running_stats count() | limit 30`, `* | sort by (x) | running_stats count(*) as "count(*)" | limit 30`)
	f(`* | sort by (x) | unroll by (a) | limit 30`, `* | sort by (x) | unroll by (a) | limit 30`)
	f(`* | fields a | limit 30`, `* | fields a | limit 30`)
...
func TestPipeIsOneToOneRowLocalTransform(t *testing.T) {
	f := func(qStr string, resultExpected bool) {
		t.Helper()

		pipeStr := strings.TrimPrefix(qStr, "* | ")
		p := mustParsePipe(pipeStr, 0)
		result := p.isOneToOneRowLocalTransform()
		if result != resultExpected {
			t.Fatalf("unexpected result for isOneToOneRowLocalTransform(%q); got %v; want %v", qStr, result, resultExpected)
		}
	}

	f("* | coalesce (x)", true)
	f("* | collapse_nums", true)
	f("* | copy foo as bar", true)
	f("* | decolorize", true)
	f("* | delete foo", true)
	f("* | drop_empty_fields", false)
	f("* | extract '<foo>bar'", true)
	f("* | extract '<foo>bar' keep_original_fields", false)
	f("* | extract '<foo>bar' skip_empty_results", false)
	f("* | extract_regexp 'foo(?P<bar>baz)'", true)
	f("* | extract_regexp 'foo(?P<bar>baz)' keep_original_fields", false)
	f("* | extract_regexp 'foo(?P<bar>baz)' skip_empty_results", false)
	f("* | fields foo", true)
	f("* | format 'foo<bar>'", true)
	f("* | hash(foo)", true)
	f("* | json_array_concat", true)
	f("* | json_array_len(foo)", true)
	f("* | len(foo)", true)
	f("* | math foo + bar as baz", true)
	f("* | pack_json", true)
	f("* | pack_logfmt", true)
	f("* | rename foo as bar", true)
	f("* | replace ('foo', 'bar')", true)
	f("* | replace_regexp ('foo', 'bar')", true)
	f("* | set_stream_fields foo", true)
	f("* | split ','", true)
	f("* | time_add 1h", true)
	f("* | unpack_json", true)
	...
	f("* | facets", false)
	f("* | field_names", false)
	f("* | field_values foo", false)
	f("* | filter foo:bar", false)
	f("* | first 10 by (foo)", false)
	f("* | generate_sequence 10", false)
	f("* | join by (foo) (*)", false)
	f("* | last 10 by (foo)", false)
	f("* | limit 10", false)
	f("* | offset 10", false)
	f("* | query_stats", false)
	...
	f("* | top 10 by (foo)", false)
	f("* | total_stats count()", false)
	f("* | union (*)", false)
	f("* | uniq by (foo)", false)
	f("* | unroll by (foo)", false)
}
...
func TestQueryCanReturnLastNResults(t *testing.T) {
	...
	f("* | coalesce (x)", true)
	f("* | collapse_nums", true)
	f("* | collapse_nums at _time", false)
	f("* | copy foo bar", true)
	f("* | copy * as foo*", true)
	...
	f("* | extract '<_time>bar<baz>' keep_original_fields", true)
	f("* | extract_regexp 'foo(?P<bar>baz)'", true)
	f("* | extract_regexp 'foo(?P<_time>baz)'", false)
	f("* | extract_regexp 'foo(?P<_time>baz)' keep_original_fields", true)
	f("* | facets", false)
	f("* | field_names", false)
	...
	f("* | math x+y as z, 10 as _time", false)
	f("* | offset 10", false)
	f("* | pack_json", true)
	...
	f("* | replace ('foo', 'bar')", true)
	f("* | replace ('foo', 'bar') at _time", false)
	f("* | replace_regexp ('foo', 'bar')", true)
	f("* | replace_regexp ('foo', 'bar') at _time", false)
	f("* | running_stats count()", false)
	f("* | sample 10", false)
	f("* | sort by (x)", false)
	f("* | split ' '", true)
	...
	f("* | union (x)", false)
	f("* | uniq (x)", false)
	// Wildcard field filters at unpack pipes may overwrite the _time field depending on the unpacked data.
	// They intentionally keep the optimization in order to not penalize the common case
	// when the unpacked data doesn't contain the _time field.
	f("* | unpack_json x", true)
	f("* | unpack_json fields (foo, _time)", false)
	f("* | unpack_json fields (_time) keep_original_fields", true)
	f("* | unpack_json fields (foo)", true)
	f("* | unpack_json result_prefix x_", true)
	f("* | unpack_json result_prefix _", true)
	f("* | unpack_json fields (time) result_prefix _", false)
	f("* | unpack_json fields (tim*) result_prefix _", true)
	f("* | unpack_logfmt x", true)
	f("* | unpack_logfmt fields (_time)", false)
	f("* | unpack_logfmt fields (_time) keep_original_fields", true)
	f("* | unpack_logfmt fields (time) result_prefix _", false)
	f("* | unpack_syslog x", true)
	f("* | unpack_words x", true)
	f("* | unroll by (x)", true)
	f("* | unroll by (x, _time)", false)
