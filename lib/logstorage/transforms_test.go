...
do baz;`, [][]Field{
	...
	// Must not restore default _msg value.
	f(`delete _msg;`, [][]Field{
		{{"_msg", ""}},
		{{"_msg", "foobar"}},
		{{"level", "info"}},
...
func TestTransformsProgramSetStream(t *testing.T) {
	f := func(program string, row []Field, rowsExpected string) {
		t.Helper()

		var rowsGot string
		store := func(lr *LogRows) {
			lr.ForEachRow(func(_ uint64, r *InsertRow) {
				...
				rowGot := string(MarshalFieldsToJSON(nil, fields))
				rowsGot += rowGot + "\n"
			})
			rowsGot = strings.Trim(rowsGot, "\n")
		}

		lr := GetLogRows(nil, nil, nil, nil, "")
		defer PutLogRows(lr)
		lr.MustAdd(TenantID{}, 0, row, -1)

		prog, err := ParseTransformsProgram(program)
		if err != nil {
			t.Fatal(err)
		}
		tr := prog.NewTransformer(store)
		tr.Transform(lr)

		if rowsGot != rowsExpected {
			t.Fatalf("unexpected rows\ngot:\n%s\nwant:\n%s", rowsGot, rowsExpected)
		}
	}

	// Set _stream field from existing fields.
	f(`set_stream_fields host, app;`, []Field{
		{"_msg", "foo"}, {"host", "bar"}, {"app", "VictoriaLogs"},
	}, `{"_msg":"foo","_stream":"{app=\"VictoriaLogs\",host=\"bar\"}","app":"VictoriaLogs","host":"bar"}`)

	// Set _stream field from non-existing fields.
	f(`set_stream_fields does_not_exist;`, []Field{
		{"_msg", "foo"}, {"host", "bar"}, {"app", "VictoriaLogs"},
	}, `{"_msg":"foo","_stream":"{}","app":"VictoriaLogs","host":"bar"}`)

	// Set empty _stream when the _stream field is removed.
	f(`delete _stream;`, []Field{
		{"_msg", "foo"}, {"host", "bar"}, {"app", "VictoriaLogs"},
	}, `{"_msg":"foo","_stream":"{}","app":"VictoriaLogs","host":"bar"}`)

	// Preserve _stream field from original row.
	f(`format pong as ping;`, []Field{
		{"_msg", "foo"}, {"_stream", `{app="VictoriaLogs",host="bar"}`}, {"host", "bar"}, {"app", "VictoriaLogs"},
	}, `{"_msg":"foo","_stream":"{app=\"VictoriaLogs\",host=\"bar\"}","app":"VictoriaLogs","host":"bar","ping":"pong"}`)

	// Normalize newly created _stream field after field removal.
	f(`set_stream_fields host, app | delete app;`, []Field{
		{"_msg", "foo"}, {"host", "bar"}, {"app", "VictoriaLogs"},
	}, `{"_msg":"foo","_stream":"{host=\"bar\"}","host":"bar"}`)

	// Normalize original _stream field after field removal.
	f(`delete app;`, []Field{
		{"_msg", "foo"}, {"_stream", `{app="VictoriaLogs",host="bar"}`}, {"host", "bar"}, {"app", "VictoriaLogs"},
	}, `{"_msg":"foo","_stream":"{host=\"bar\"}","host":"bar"}`)
}
