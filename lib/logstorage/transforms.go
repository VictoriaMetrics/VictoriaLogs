...
func (tp *TransformsProgram) NewTransformer(flush func(lr *LogRows)) *Transformer {
	...
	storeResult := func(workerID uint, br *blockResult) {
		...
		if shard.lr == nil {
			shard.lr = GetLogRows(nil, nil, nil, nil, "")
		}
		lr := shard.lr
...
func parseTransformBlockName(lex *lexer) (string, error) {
	...
	if lex.isQuotedToken() {
		return "", fmt.Errorf("block name cannot be quoted")
	}

	name := lex.token
	// LogsQL keywords are case-insensitive, while custom block names are not.
	if namelc := strings.ToLower(name); slices.Contains(transformsKeywords, namelc) || isPipeName(namelc) {
	...
	if r, _ := utf8.DecodeRuneInString(name); unicode.IsDigit(r) {
		return "", fmt.Errorf("block name cannot be started with a digit, got %q", name)
	}
