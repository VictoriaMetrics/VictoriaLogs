...
func (br *blockResult) mustInitFromLogRows(lr *LogRows) {
	...
	st := GetStreamTags()
	valuesBufLen := len(br.valuesBuf)
	for _, stc := range lr.streamTagsCanonicals {
	...
	for i := range lr.streamIDs {
		...
		accountIDStr := bytesutil.ToUnsafeString(bb.B)
		br.addValue(accountIDStr)
	}
	...
	for i := range lr.streamIDs {
		...
		projectIDStr := bytesutil.ToUnsafeString(bb.B)
		br.addValue(projectIDStr)
	}
