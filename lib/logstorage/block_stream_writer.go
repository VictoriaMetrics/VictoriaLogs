...
func (w *bloomValuesWriter) appendCloserTasks(pe *fsutil.ParallelExecutor) {
	pe.Add(fs.NewMustCloserTask(&w.bloom))
	pe.Add(fs.NewMustCloserTask(&w.values))
}
...
func (sw *streamWriters) MustClose() {
	...
	pe.Add(fs.NewMustCloserTask(&sw.columnNamesWriter))
	pe.Add(fs.NewMustCloserTask(&sw.columnIdxsWriter))
	pe.Add(fs.NewMustCloserTask(&sw.metaindexWriter))
	pe.Add(fs.NewMustCloserTask(&sw.indexWriter))
	pe.Add(fs.NewMustCloserTask(&sw.columnsHeaderIndexWriter))
	pe.Add(fs.NewMustCloserTask(&sw.columnsHeaderWriter))
	pe.Add(fs.NewMustCloserTask(&sw.timestampsWriter))
