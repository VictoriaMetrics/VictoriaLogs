...
func TestSplitQueryToRemoteAndLocal(t *testing.T) {
	...
	f(`foo | facets`, `foo | facets 18446744073709551615 | fields field_name, field_value, hits`, `stats by (field_name, field_value) sum(hits) as hits | total_stats by (field_name) count(*) as field_values_count | filter field_values_count:<=1000 | sort by (hits desc) partition by (field_name) limit 10 | sort by (field_name, hits desc) | fields field_name, field_value, hits`)
