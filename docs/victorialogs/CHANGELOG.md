...
**Update Note 1:** [LogsQL](https://docs.victoriametrics.com/victorialogs/logsql/): `switch` is now parsed as the start of a `switch(...)` construct when it immediately follows `format` in the [`format` pipe](https://docs.victoriametrics.com/victorialogs/logsql/#format-pipe). This may be a breaking change if you have queries with an unquoted `format` pattern starting with `switch` in that position, such as `... | format switch-case as x` - such patterns must be quoted now: `... | format "switch-case" as x`.
...
* FEATURE: [LogsQL](https://docs.victoriametrics.com/victorialogs/logsql/): add `switch(...)` syntax to the [`format` pipe](https://docs.victoriametrics.com/victorialogs/logsql/#format-pipe) for formatting based on multiple conditions. See [#1255](https://github.com/VictoriaMetrics/VictoriaLogs/issues/1255).
