...
* FEATURE: [querying API](https://docs.victoriametrics.com/victorialogs/querying/): set the `VL-Partial-Response` HTTP header in responses from [query endpoints](https://docs.victoriametrics.com/victorialogs/querying/#http-api). The header is set to `true` if the [response is partial](https://docs.victoriametrics.com/victorialogs/querying/#partial-responses) and to `false` if the response is complete. The header is set to `unknown` for streaming queries, which do not buffer the final result in memory. See [#718](https://github.com/VictoriaMetrics/VictoriaLogs/issues/718).
