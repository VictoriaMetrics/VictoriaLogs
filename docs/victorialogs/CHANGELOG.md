...
* BUGFIX: [live tailing](https://docs.victoriametrics.com/victorialogs/querying/#live-tailing): return a clear HTTP error when the initial query fails, including when its results have a missing or invalid `_time` field. Previously, these errors could produce an invalid HTTP response, which proxies such as `vmauth` could return as `502 Bad Gateway`. See [#1761](https://github.com/VictoriaMetrics/VictoriaLogs/issues/1761).
