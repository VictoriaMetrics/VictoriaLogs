...
**Update note 3:** [cluster version](https://docs.victoriametrics.com/victorialogs/cluster/): the internal RPC endpoints `/internal/insert`, `/internal/select/*` and `/internal/delete/*` are moved to the `/internal/rpc/*` path prefix, so they can be protected with a single [vmauth](https://docs.victoriametrics.com/victoriametrics/vmauth/) rule - see [these docs](https://docs.victoriametrics.com/victorialogs/security-and-lb/#internal-rpc-authorization). This release still accepts the old paths, so upgrade `vlstorage` nodes first and then `vlselect` and `vlinsert` nodes in order to avoid downtime. The old paths will be removed in the next release: upgrade all the cluster components to this release before upgrading to any newer release. The `path` label of HTTP metrics for these endpoints now contains the new paths.
