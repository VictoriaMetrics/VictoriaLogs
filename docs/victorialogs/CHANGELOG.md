...
* BUGFIX: [cluster version](https://docs.victoriametrics.com/victorialogs/cluster/): `vlstorage` now drops queued `/internal/select/*` requests when `vlselect` has already disconnected. Previously the request body was read only after a free slot in the `-internalselect.maxConcurrentRequests` limiter was obtained, so the closed client connection stayed undetected. Such abandoned requests kept piling up in the wait queue, and were additionally executed in full once a free slot appeared, even though their responses could no longer be written to the disconnected `vlselect`. This wasted concurrency slots and CPU on an already busy `vlstorage`, delaying the queries which were still waited for, and inflated `vl_http_request_duration_seconds{path="/internal/select/*"}` up to minutes. See [#1554](https://github.com/VictoriaMetrics/VictoriaLogs/issues/1554).
