...
* FEATURE: [cluster version](https://docs.victoriametrics.com/victorialogs/cluster/): add `-internalselect.maxQueueDuration` command-line flag, which limits the time an `/internal/select/*` request spends in the wait queue when the `-internalselect.maxConcurrentRequests` limit is reached. Previously such requests stayed in the queue without timeout. See [#1672](https://github.com/VictoriaMetrics/VictoriaLogs/issues/1672).
