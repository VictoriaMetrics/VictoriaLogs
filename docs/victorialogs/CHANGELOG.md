...
* BUGFIX: [vlinsert](https://docs.victoriametrics.com/victorialogs/cluster/): a slow or unresponsive `vlstorage` node no longer blocks data ingestion to the healthy nodes. `vlinsert` re-routes an insert request to other nodes when the node already handles `-insert.concurrency` concurrent requests or does not respond within one minute. The new `vl_insert_remote_concurrency_limit_reached_total` metric counts re-routing events caused by reaching the concurrency limit. See [#1512](https://github.com/VictoriaMetrics/VictoriaLogs/issues/1512).
