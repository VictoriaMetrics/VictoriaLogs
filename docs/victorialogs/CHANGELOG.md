...
* BUGFIX: [LogsQL](https://docs.victoriametrics.com/victorialogs/logsql/): apply the `limit` from a trailing [`limit` pipe](https://docs.victoriametrics.com/victorialogs/logsql/#limit-pipe) to the preceding [`sort` pipe](https://docs.victoriametrics.com/victorialogs/logsql/#sort-pipe) when the pipes between them write a single output row per every input row. Previously queries such as `* | sort by (_time) desc | pack_json` could fail with the `cannot calculate [...], since it requires more than NMB of memory` error, while the equivalent `* | pack_json | sort by (_time) desc` succeeded. See [#1601](https://github.com/VictoriaMetrics/VictoriaLogs/issues/1601).
