...
* BUGFIX: [VictoriaLogs cluster](https://docs.victoriametrics.com/victorialogs/cluster/): prevent possible `EOF` and `connection reset by peer` errors when `vlinsert` or `vlselect` reuse idle connections to `vlstorage` nodes, which have been already closed by `vlstorage`. See [#1440](https://github.com/VictoriaMetrics/VictoriaLogs/issues/1440).
