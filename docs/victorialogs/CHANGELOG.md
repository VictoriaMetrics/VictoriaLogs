...
* FEATURE: [web UI](https://docs.victoriametrics.com/victorialogs/querying/#web-ui): show the `limit` and time range actually used to produce the results, when a reverse proxy in front of VictoriaLogs reports different values via the `VL-Applied-Limit`, `VL-Applied-Start` and `VL-Applied-End` response headers (e.g. because it clamped these parameters to protect a shared instance).
