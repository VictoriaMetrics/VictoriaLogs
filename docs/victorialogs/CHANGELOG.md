...
* BUGFIX: [syslog data ingestion](https://docs.victoriametrics.com/victorialogs/data-ingestion/syslog/): properly handle syslog messages sent over TCP that contain a newline character inside the message. Previously such a message could cause the next message to be parsed incorrectly. See [#1365](https://github.com/VictoriaMetrics/VictoriaLogs/issues/1365).
