...
* BUGFIX: [metrics](https://docs.victoriametrics.com/victorialogs/metrics/): prevent `vl_streams_created_total` at `vlstorage` from decreasing when an old daily partition is removed. The metric now increases until `vlstorage` restarts, so Prometheus [`increase()`](https://docs.victoriametrics.com/metricsql/#increase) no longer reports false spikes after old partitions are deleted. See [#1461](https://github.com/VictoriaMetrics/VictoriaLogs/issues/1461).
