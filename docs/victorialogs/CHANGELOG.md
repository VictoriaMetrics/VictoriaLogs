...
* BUGFIX: [HTTP querying APIs](https://docs.victoriametrics.com/victorialogs/querying/#http-api): properly expose the request body size exceeded error for `vlstorage` and add a config option. Previously, a protocol version error was returned because the version is read from the form body, with the read limit hardcoded to 10MB. See [#1462](https://github.com/VictoriaMetrics/VictoriaLogs/issues/1462).
