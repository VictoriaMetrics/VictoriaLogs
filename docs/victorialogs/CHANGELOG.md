...
**Update note 1:** the `/internal/force_merge`, `/internal/force_flush`, `/internal/log_new_streams` and `/internal/partition/*` HTTP endpoints now require the `POST` method. Update any scripts or automation calling these endpoints via `GET` to use `POST`.

**Update note 2:** [cluster version](https://docs.victoriametrics.com/victorialogs/cluster/): the internal RPC endpoints are moved to the dedicated `/internal/rpc/*` path prefix for better code maintainability and for protecting them in multi-level cluster setup with a single [vmauth](https://docs.victoriametrics.com/victoriametrics/vmauth/) rule - see [these docs](https://docs.victoriametrics.com/victorialogs/security-and-lb/#multi-level-cluster-authorization). This release still accepts the old paths, so upgrade `vlstorage` nodes first and then `vlselect` and `vlinsert` nodes in order to avoid downtime. The old paths will be removed in the next release: upgrade all the cluster components to this release before upgrading to any newer release. The `path` label of HTTP metrics for these endpoints now contains the new paths.

**Update note 3:** requests to `/select/vmalert/-/reload` and other `/select/vmalert/*` paths ending with `/config` or `/reload` now require the `-httpAuth.*` credentials. Add the credentials to any scripts calling these paths.
...
* SECURITY: check the `-httpAuth.*` credentials for requests to paths ending with `/config` or `/reload`. Previously, HTTP Basic Auth was skipped for such requests, so `/select/vmalert/-/reload` was proxied to vmalert without checking the credentials. See [VictoriaMetrics#11548](https://github.com/VictoriaMetrics/VictoriaMetrics/pull/11548).

* FEATURE: support for accepting HTTP requests over Unix domain socket via `-httpListenAddr=unix:/path/to/socket` command-line flag. The socket file is created with `0600` permissions, so only the user running the VictoriaLogs component can connect to it. Note that `-tls` and `-httpListenAddr.useProxyProtocol` cannot be used with Unix domain sockets. See [#1618](https://github.com/VictoriaMetrics/VictoriaLogs/issues/1618).
...
* FEATURE: [cluster version](https://docs.victoriametrics.com/victorialogs/cluster/): allow protecting all the internal RPC endpoints with a single [vmauth](https://docs.victoriametrics.com/victoriametrics/vmauth/) rule by moving them from `/internal/*` to the dedicated `/internal/rpc/*` path prefix. See [these docs](https://docs.victoriametrics.com/victorialogs/security-and-lb/#multi-level-cluster-authorization) and [#1748](https://github.com/VictoriaMetrics/VictoriaLogs/pull/1748).
...
* BUGFIX: [File Collector](https://docs.victoriametrics.com/victorialogs/vlagent/#collect-logs-from-files): drop the incomplete last log line with a warning when the log file is deleted. Previously, `vlagent` treated this case as a bug and panicked. See [#1819](https://github.com/VictoriaMetrics/VictoriaLogs/pull/1819).
