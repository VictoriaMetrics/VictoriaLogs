...
* BUGFIX: [Syslog data ingestion](https://docs.victoriametrics.com/victorialogs/data-ingestion/syslog/): keep parsing [CEF messages](https://docs.victoriametrics.com/victorialogs/data-ingestion/syslog/#cef) which contain an unescaped `=` inside an extension field value, such as the `=` padding at the end of a base64 value. Previously such a message lost every `cef.*` field, was stored as raw text and landed in a different [log stream](https://docs.victoriametrics.com/victorialogs/data-ingestion/syslog/#stream-fields). See [#1731](https://github.com/VictoriaMetrics/VictoriaLogs/issues/1731).
