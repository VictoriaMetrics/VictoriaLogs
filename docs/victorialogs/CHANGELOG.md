...
* BUGFIX: [data ingestion](https://docs.victoriametrics.com/victorialogs/data-ingestion/): increment [`vl_http_errors_total`](https://docs.victoriametrics.com/victorialogs/metrics/#vl_http_errors_total) for failed insert requests, such as requests with invalid params or requests rejected because the storage is in read-only mode (e.g. because of lack of free disk space). Also expose the missing `vl_http_errors_total` for Loki, Elasticsearch and DataDog ingestion endpoints. See [#1811](https://github.com/VictoriaMetrics/VictoriaLogs/pull/1811).
