...
**Update Note 1:** [LogsQL](https://docs.victoriametrics.com/victorialogs/logsql/): `switch` is now parsed as the start of a `switch(...)` construct when it immediately follows `format` in the [`format` pipe](https://docs.victoriametrics.com/victorialogs/logsql/#format-pipe). This may be a breaking change if you have queries using `switch` as a literal `format` pattern in that position, such as `... | format switch as x` - it must be quoted now: `... | format "switch" as x`.
