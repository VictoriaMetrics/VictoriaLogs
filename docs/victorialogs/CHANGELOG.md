...
* BUGFIX: [data ingestion](https://docs.victoriametrics.com/victorialogs/data-ingestion/): properly handle log entries with duplicate field names, according to [data model](https://docs.victoriametrics.com/victorialogs/keyconcepts/#data-model) - the first non-empty value is stored now, and the same value is used for the corresponding [stream field](https://docs.victoriametrics.com/victorialogs/keyconcepts/#stream-fields). Previously such fields could be returned multiple times in query results, while the log entry could be associated with an unexpected [log stream](https://docs.victoriametrics.com/victorialogs/keyconcepts/#stream-fields). See [#1286](https://github.com/VictoriaMetrics/VictoriaLogs/issues/1286) and [#1603](https://github.com/VictoriaMetrics/VictoriaLogs/issues/1603).
