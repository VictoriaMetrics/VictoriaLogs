...
* FEATURE: [data ingestion](https://docs.victoriametrics.com/victorialogs/data-ingestion/): add `-insert.maxLogsPerSecond` and `-insert.maxBytesPerSecond` command-line flags for limiting the global data ingestion rate across all the supported data ingestion protocols. Both limits are disabled by default. HTTP-based data ingestion requests are rejected with the `429 Too Many Requests` status code and the `Retry-After` response header when the limits are exceeded, while non-HTTP protocols such as syslog are throttled. See [these docs](https://docs.victoriametrics.com/victorialogs/data-ingestion/#rate-limiting) and [#887](https://github.com/VictoriaMetrics/VictoriaLogs/issues/887).
