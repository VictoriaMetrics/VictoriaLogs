...
* FEATURE: [vlagent](https://docs.victoriametrics.com/victorialogs/vlagent/): add `-remoteWrite.retryOn404` command-line flag for retrying sending a block of data to the corresponding `-remoteWrite.url` when a 404 status code is received instead of dropping the block. See [#1499](https://github.com/VictoriaMetrics/VictoriaLogs/issues/1499).
