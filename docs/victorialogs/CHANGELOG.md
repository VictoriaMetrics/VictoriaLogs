...
**Update note 1:** the base Docker image has been changed from Alpine to `distroless`. If you relied on Alpine-specific tools or shell access inside the container, it is recommended to use Alpine-based image directly instead. For debugging in Kubernetes it is recommended to use `kubectl debug`.
**Update note 2:** VictoriaLogs no longer provides a Docker image for the `linux/386` platform. Users can build the binary for this platform themselves using the `make <appname>-linux-386-prod` command.
...
* FEATURE: switch base Docker image from Alpine to `distroless` for VictoriaLogs, `vlagent` and `vlogscli`. This reduces the image size and attack surface. See [#1228](https://github.com/VictoriaMetrics/VictoriaLogs/issues/1228).
