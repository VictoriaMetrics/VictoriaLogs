...
* FEATURE: add `-healthcheck` command-line flag to [VictoriaLogs](https://docs.victoriametrics.com/victorialogs/), which performs a healthcheck of the locally running VictoriaLogs by querying the `/health` endpoint and exits with `0` status code on 2xx responses and `1` otherwise. The flag is used by the built-in `HEALTHCHECK` instruction in the Docker image. See [#1565](https://github.com/VictoriaMetrics/VictoriaLogs/issues/1565).
