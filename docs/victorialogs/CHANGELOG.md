...
* BUGFIX: [Elasticsearch data ingestion](https://docs.victoriametrics.com/victorialogs/data-ingestion/#elasticsearch-bulk-api): properly report log entries exceeding `-insert.maxLineSizeBytes` as failed items in the `_bulk` response, while continuing to ingest valid log entries from the same request. Previously such log entries could be skipped without being reported in the `_bulk` response.
