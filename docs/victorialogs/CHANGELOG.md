...
* FEATURE: [data ingestion](https://docs.victoriametrics.com/victorialogs/data-ingestion/): attempt to restore invalid `_stream` value instead of dropping the log. If a `_stream` tag name is missing from the fields, it is removed from the `_stream` fields. If a `_stream` tag value differs from the one in the fields, it is updated to match. This simplifies importing of previously modified VictoriaLogs-exported data. See [#1122](https://github.com/VictoriaMetrics/VictoriaLogs/issues/1122).
