...
* BUGFIX: [cluster version](https://docs.victoriametrics.com/victorialogs/cluster/): properly check for non-unavailable errors when querying `vlstorage` nodes. Previously, such errors were silently ignored, which could mask configuration issues.
