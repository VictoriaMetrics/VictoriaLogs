...
* FEATURE: [querying API](https://docs.victoriametrics.com/victorialogs/querying/): add `/select/logsql/debug` endpoint for testing LogsQL queries against user-provided sample logs without storing data. This endpoint accepts a JSON payload with a query and sample logs, validates the query syntax, and returns results with execution statistics. This significantly lowers the barrier to entry for learning LogsQL and speeds up query development. See [these docs](https://docs.victoriametrics.com/victorialogs/querying/#logsql-debugger) and [#1250](https://github.com/VictoriaMetrics/VictoriaLogs/issues/1250).
