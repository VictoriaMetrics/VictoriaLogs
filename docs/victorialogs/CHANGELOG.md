...
* FEATURE: [metrics](https://docs.victoriametrics.com/victorialogs/metrics/): expose the number of [partition snapshots](https://docs.victoriametrics.com/victorialogs/#backup-and-restore) via the `vl_snapshots` metric, the number of requests to `/internal/partition/snapshot/create` and `/internal/partition/snapshot/delete` via the `vl_http_requests_total` metric, and the number of failed requests to delete snapshots via the `vl_http_errors_total{path="/internal/partition/snapshot/delete"}` metric. This allows monitoring backups and detecting snapshots. See [#1815](https://github.com/VictoriaMetrics/VictoriaLogs/issues/1815).
