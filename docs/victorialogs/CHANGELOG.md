...
* FEATURE: [web UI](https://docs.victoriametrics.com/victorialogs/querying/#web-ui): display human-readable tenant names at the tenant selector. The names are set via the `-vmui.tenantAliases` command-line flag, e.g. `-vmui.tenantAliases='0:0=k8s,0:1=nginx-access'`. See [these docs](https://docs.victoriametrics.com/victorialogs/#multitenancy) and [#1703](https://github.com/VictoriaMetrics/VictoriaLogs/issues/1703).
