...
* BUGFIX: [data ingestion](https://docs.victoriametrics.com/victorialogs/data-ingestion/): consistently preserve the first value and drop subsequent values when a log entry contains duplicate field names. A throttled warning is logged for the dropped duplicate fields. Previously, duplicate fields could be stored and the visible value could change after background merges. See [#1286](https://github.com/VictoriaMetrics/VictoriaLogs/issues/1286).
