...
* BUGFIX: [syslog data ingestion](https://docs.victoriametrics.com/victorialogs/data-ingestion/syslog/): split syslog messages containing newline characters into separate log entries at every newline when they are ingested via TCP, so a message with an embedded newline no longer corrupts the parsing of the following messages. See [#1365](https://github.com/VictoriaMetrics/VictoriaLogs/issues/1365).
