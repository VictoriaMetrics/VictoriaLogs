...
* FEATURE: [data ingestion](https://docs.victoriametrics.com/victorialogs/data-ingestion/): add optional `-insert.discoverLogLevels` and `-insert.logLevelFields` flags for discovering a common `level` field from configurable candidate fields without overwriting existing `level` values. See [#1402](https://github.com/VictoriaMetrics/VictoriaLogs/issues/1402).
