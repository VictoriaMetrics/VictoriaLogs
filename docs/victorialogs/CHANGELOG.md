...
* BUGFIX: [web UI](https://docs.victoriametrics.com/victorialogs/querying/#web-ui): stop adding a browser history entry on every auto-refresh tick. Previously, each refresh of a relative time range rewrote the time query args, so an idle tab with a short refresh interval could add thousands of entries per hour and make the browser `Back` button unusable. See [#1676](https://github.com/VictoriaMetrics/VictoriaLogs/pull/1676).
