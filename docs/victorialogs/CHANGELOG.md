...
* FEATURE: [querying API](https://docs.victoriametrics.com/victorialogs/querying/): add `sort_direction` query arg to [`/select/logsql/query`](https://docs.victoriametrics.com/victorialogs/querying/#querying-logs). Pass `sort_direction=asc` together with `limit=N` to return the `N` oldest matching log entries instead of the `N` most recent ones. See [#924](https://github.com/VictoriaMetrics/VictoriaLogs/issues/924).
