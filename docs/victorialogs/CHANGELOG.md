...
* FEATURE: [Docker images](https://docs.victoriametrics.com/victorialogs/quickstart/#docker-image): add `-healthcheck` command-line flag to VictoriaLogs, which performs a healthcheck of the locally running VictoriaLogs by querying the `/health` endpoint and exits with `0` or `1` status code depending on the response status. The flag is used by the built-in `HEALTHCHECK` instruction in Docker images, so the container health can be inspected with tools such as `docker ps` without needing `curl` or `wget` inside the container. The built-in healthcheck reads flags from environment variables via `-envflag.enable`, so it automatically follows `-httpListenAddr`, `-tls` and `-http.pathPrefix` when VictoriaLogs is configured via environment variables; it cannot be used when `-httpListenAddr.useProxyProtocol` is enabled for the first `-httpListenAddr`. The flag can also be used in Kubernetes [exec probes](https://kubernetes.io/docs/tasks/configure-pod-container/configure-liveness-readiness-startup-probes/#define-a-liveness-command). See [#1565](https://github.com/VictoriaMetrics/VictoriaLogs/issues/1565).
