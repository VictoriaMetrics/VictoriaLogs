...
* FEATURE: [VictoriaLogs](https://docs.victoriametrics.com/victorialogs/) and [vlagent](https://docs.victoriametrics.com/victorialogs/vlagent/): add `-insert.maxLogsPerSecond` and `-insert.maxBytesPerSecond` command-line flags for limiting the global rate of the data ingested via the [supported data ingestion protocols](https://docs.victoriametrics.com/victorialogs/data-ingestion/). Both limits are disabled by default. The ingestion is throttled when any of the configured limits is exceeded. See [these docs](https://docs.victoriametrics.com/victorialogs/data-ingestion/#rate-limiting) and [#887](https://github.com/VictoriaMetrics/VictoriaLogs/issues/887).
