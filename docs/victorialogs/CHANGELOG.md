...
* BUGFIX: [Loki data ingestion](https://docs.victoriametrics.com/victorialogs/data-ingestion/promtail/): ignore whitespace around label names received via the Loki protobuf API. Previously, whitespace after the opening `{` could become part of the first label name, so the label couldn't be queried by its expected name. See [#1666](https://github.com/VictoriaMetrics/VictoriaLogs/issues/1666).
