...
* BUGFIX: [HTTP querying APIs](https://docs.victoriametrics.com/victorialogs/querying/#http-api): properly return `request body too large` error when a cluster query sends too much data from `vlselect` to `vlstorage`. Previously such queries could incorrectly fail with `unexpected protocol version=""`, which looked like a version mismatch between cluster components. See [#1462](https://github.com/VictoriaMetrics/VictoriaLogs/issues/1462).
