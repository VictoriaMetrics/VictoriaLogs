...
* FEATURE: [Kubernetes Collector](https://docs.victoriametrics.com/victorialogs/vlagent/#collect-kubernetes-pod-logs): add a new field `_output_stream` that indicates whether CRI log lines are from stdout or stderr, allowing logs to be quickly filtered by output stream at query time. See [#1790](https://github.com/VictoriaMetrics/VictoriaLogs/issues/1790)
