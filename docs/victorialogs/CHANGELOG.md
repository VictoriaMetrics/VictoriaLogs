...
* BUGFIX: [live tailing](https://docs.victoriametrics.com/victorialogs/querying/#live-tailing): immediately show logs from [log streams](https://docs.victoriametrics.com/victorialogs/keyconcepts/#stream-fields) created after live tailing has started, when the query uses a [stream filter](https://docs.victoriametrics.com/victorialogs/logsql/#stream-filter). Previously logs from such newly created streams could be missing from the `/select/logsql/tail` output for up to ~10 seconds. See [#1477](https://github.com/VictoriaMetrics/VictoriaLogs/issues/1477).
