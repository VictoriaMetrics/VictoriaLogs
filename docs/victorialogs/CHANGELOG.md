...
* FEATURE: add [multitenant querying](https://docs.victoriametrics.com/victorialogs/#multitenant-querying) via `/select/multitenant/logsql/*` endpoints for querying logs across multiple tenants in a single request. Enable these endpoints with `-multitenantselect.enable` and filter by `vl_account_id` / `vl_project_id` fields in LogsQL to select the needed tenants. These fields are also returned in query results unless they are removed by LogsQL pipes. In [cluster mode](https://docs.victoriametrics.com/victorialogs/cluster/), upgrade all queried `vlstorage` and `vlselect` nodes before using multitenant querying, since older nodes do not support the new internal endpoints. Regular queries remain compatible with the previous internal protocol. See [#91](https://github.com/VictoriaMetrics/VictoriaLogs/issues/91).
