...
See the docs at https://docs.victoriametrics.com/victorialogs/
  ...
  -httpListenAddr array
     TCP address (e.g. :9428) or Unix domain socket (e.g. unix:/run/victoria-logs.sock) to listen for incoming http requests. See also -tls and -httpListenAddr.useProxyProtocol, which are supported only for TCP addresses
     Supports an array of values separated by comma or specified via multiple flags.
     Each array item can contain comma inside single-quoted or double-quoted string, {}, [] and () braces.
