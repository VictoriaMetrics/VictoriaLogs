...
If `vl_account_id` and `vl_project_id` fields arrive in the incoming log while log transformations are running,
`vlagent` strips them from the result and ignores them for tenant routing, so the target tenant is not changed.
To avoid this, explicitly set these fields during log transformations to override the target tenant. This only
applies when a transformation program is configured - without one, these fields are inserted as ordinary fields.
