...
Sometimes it may be useful to attach an example log line to the alert message (e.g. to see a representative error in Slack without opening UI). Use [`row_any()`](https://docs.victoriametrics.com/victorialogs/logsql/#row_any-stats) only inside `annotations` via the `query` template function.

Do not use these functions in `expr`, since the returned row can change between evaluations. Since vmalert identifies each alert instance by the full label set (excluding `__name__`), changing labels leads to alert flapping and resets the `for:` timer.
