...
import { getFromStorage, saveToStorage, ActiveStorageKeys } from "../utils/storage";
import useEventListener from "./useEventListener";

/**
 * A custom hook that synchronizes a boolean state with a value stored in localStorage.
 *
 * @param {ActiveStorageKeys} key - The key used to access the corresponding value in localStorage.
 * @param {(key: ActiveStorageKeys) => boolean} [customGetter] - Optional custom getter function used to read
...
export const useLocalStorageBoolean = (
  key: ActiveStorageKeys,
  customGetter?: (key: ActiveStorageKeys) => boolean
): [boolean, (value: boolean) => void] => {
  const getter = useCallback((key: ActiveStorageKeys) => {
