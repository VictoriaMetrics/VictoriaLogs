...
describe("useHideChart", () => {
  ...
  it("removes hide_chart from the URL when showing a chart hidden by the URL", () => {
    routerMock.search = "?hide_chart=true&query=error";
    const { result } = renderHook(() => useHideChart());
    ...
    const nextSearchParams = routerMock.setSearchParams.mock.calls[0][0];
    expect(nextSearchParams.has("hide_chart")).toBe(false);
    expect(nextSearchParams.get("query")).toBe("error");
  });
});
