...
func (bsw *blockStreamWriter) MustClose() {
	...
	// on high-latency storage systems such as NFS or Ceph.
	var pe fsutil.ParallelExecutor
	pe.Add(fs.NewMustCloserTask(bsw.metaindexWriter))
	pe.Add(fs.NewMustCloserTask(bsw.indexWriter))
	pe.Add(fs.NewMustCloserTask(bsw.itemsWriter))
	pe.Add(fs.NewMustCloserTask(bsw.lensWriter))
	pe.Run()
